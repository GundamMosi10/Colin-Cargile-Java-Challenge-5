package com.m5challenge.gamestoreinvoicing.service;

import com.m5challenge.gamestoreinvoicing.util.feign.GameStoreClient;
...
import com.m5challenge.gamestoreinvoicing.repository.*;
import com.m5challenge.gamestoreinvoicing.model.*;
import com.m5challenge.gamestoreinvoicing.viewModel.InvoiceViewModel;
...
public class GameStoreInvoiceServiceLayer {
    ...
    public InvoiceViewModel createInvoice(InvoiceViewModel invoiceViewModel) {
        ...
        if (invoiceViewModel.getItemType().equals(CONSOLE_ITEM_TYPE)) {
            Console returnVal = gameStoreClient.getConsole(invoiceViewModel.getItemId());
            if (returnVal == null) {
                throw new IllegalArgumentException("Requested console is unavailable");
            }
            ...
            Game returnVal = gameStoreClient.getGameInfo(invoiceViewModel.getItemId());
            if (returnVal == null) {
            ...
            TShirt returnVal = gameStoreClient.getTShirt(invoiceViewModel.getItemId());
            if (returnVal == null) {
                throw new IllegalArgumentException("Requested Tshirt is unavailable");
            }
            if (invoiceViewModel.getQuantity() > returnVal.getQuantity()) {
                throw new IllegalArgumentException("Requested quantity is unavailable");
            }
            ...
            throw new IllegalArgumentException(invoiceViewModel.getItemType() + ": Unrecognized Item type. Valid ones: T-Shirt, Console, or Game");
        }
        ...
        Optional<Tax> returnVal2 = taxRepo.findById(invoice.getState());

        if (returnVal2.isPresent()) {
            tempTaxRate = returnVal2.get().getRate();
        } else {
        ...
        Optional<ProcessingFee> returnVal3 = processingFeeRepo.findById(invoiceViewModel.getItemType());

        if (returnVal3.isPresent()) {
            processingFee = returnVal3.get().getFee();
        } else {
